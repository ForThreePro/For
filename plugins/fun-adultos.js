let handler = async (m, { conn, command }) => {
    ...
    // Para Nox Bot
    let name = global.db.data.users[who]?.name || conn.getName(who) || who.split('@')[0]

    switch(command) {

        case 'violar': {
            if (!m.mentionedJid[0]) return m.reply('Menciona a quien 😈\n.violar @tag')
            let name2 = global.db.data.users[m.mentionedJid[0]]?.name || conn.getName(m.mentionedJid[0]) || m.mentionedJid[0].split('@')[0]
