...
let handler = async (m, { conn, command, isAdmin }) => {
    ...
    if (command === 'siente') {
        if (salas.estado) return m.reply('❌ Ya hay una sala activa. Usa.sala pa ver o.terminar pa borrar')
    ...
    if (command === 'empezar') {
        ...
        salas.msgId = msg.key.id // GUARDAMOS EL ID DEL MSG DEL BOT. CLAVE
        return
    }

    // 6. LÓGICA DE TURNO - FIX PRINCIPAL
    if (!salas || salas.estado!== 'jugando') return
    let game = salas

    if (m.sender!== game.turno) return // Solo el del turno

    // FIX: En vez de m.quoted.fromMe, comparamos el ID del mensaje
    if (!m.quoted || m.quoted.id!== game.msgId) return // Solo si respondes al msg exacto del bot
    ...
    game.msgId = newMsg.key.id // ACTUALIZAMOS EL ID PARA EL SIGUIENTE TURNO
}
